...
function hydrateFromDoc(doc: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    name: underName?.name,
    email: contactPoint?.email ?? underName?.email,
    contactName: contactPoint?.name,
    contactTitle: (contactPoint?.contactType ?? contactPoint?.jobTitle) as string | undefined,
    ...
    intendedUse: (subj.intendedUse ?? underName?.intendedUse) as string | undefined,
    status: doc.status,
